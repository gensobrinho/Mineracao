...
class GitHubAccessibilityMiner {
    ...
    try {
      if (fs.existsSync(this.processedReposFile)) {
        const data = JSON.parse(
          fs.readFileSync(this.processedReposFile, "utf8")
        );
  ...
  async isLibraryRepository(repo) {
    ...
    if (isLibrary) {
      ...
      console.log(
        `   📚 Biblioteca detectada (${reasons.join(", ")}): ${repo.full_name}`
      );
    }
  ...
  async analyzeRepository(repo) {
    ...
    try {
      ...
      if (hasAnyTool) {
        const toolsFound = Object.keys(foundTools).filter(
          (key) => foundTools[key]
        );
  ...
  searchToolsInContent(content, foundTools) {
    ...
    for (const [toolName, keywords] of Object.entries(
      this.accessibilityTools
    )) {
